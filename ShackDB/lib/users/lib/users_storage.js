/**
 * Create and export the storage used for users metadata.
 * |-- copyright and license --|
 * @package    ShackDB
 * @file       users_storage.js
...
const {Storage, StoragePolicies} = require('../../storage');

var usersStorageDrivers;
try {
    usersStorageDrivers = config.storageDriversGenerator[config.usersStorageType]();
}
catch (e) {
    throw new Error(`Invalid or missing storage type for users repo (type = ${config.usersStorageType})`);  
}
const usersStorage = new Storage('users', config.paths.usersDataRoot, null, usersStorageDrivers, null, new StoragePolicies({}));
usersStorage.initS
